...
async function regenerateTextSuggestions() {
    ...
    if (boxdata.length > 0) {
        ...
        displayMessage({
            type: "error",
            title: "No boxes drawn",
            message: "Please first draw some boxes to generate text suggestions or use the \"Generate boxes\" button.",
        })
    }
...
function applyTransliterationRules(text, rules) {
    for (let rule of rules) {
        let originalCharacter = rule["original-character"];
        let conditions = rule["conditions"];

        for (let condition of conditions) {
            let context = condition["context"];
            let output = condition["output"];
    ...
    if (selectedBox) {
        ...
        if (boxdata.lenght > 0) {
            displayMessage({
                ...
                message: "Please select a box from the image first.",
            })
        } else {
            displayMessage({
                type: "error",
                title: "No box drawn",
                message: "Please draw at least one box first.",
            })
        }
    }
...
async function recognizeText(boxes = []) {
...
async function setMapSize(options, animate = true) {
    ...
    // textOutput.style.height = newHeight + "px";
